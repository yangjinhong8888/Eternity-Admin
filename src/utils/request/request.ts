 ...
 */
interface ReqParameter<T = unknown> {
  /**
   * 请求的URL路径
   */
  url: string
 ...
 * @template T - 请求体数据或查询参数的类型
 */
interface ReqParameterCustom<T = unknown> extends ReqParameter<T> {
  /**
...
class HttpClient {
   ...
   * @param params - 请求参数配置
   * @returns Promise<IResult<R>> - 返回响应Promise
   */
  private async request<T, R>(
    method: Method,
    params: ReqParameter<T>
  ): Promise<IResult<R>> {
    ...
    return this.axiosInstance.request<R, IResult<R>>(config)
  }
